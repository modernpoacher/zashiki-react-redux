...
export function embarkRoute (route) {
  return {
    type: ZASHIKI_EMBARK,
    promise: () => request.post('zashiki-transport/embark', route)
  }
}
...
function fetchRoute () {
  return {
    ...
    promise: () => request.get('zashiki-transport/embark/fetch')
  }
}
...
function storeRoute (route) {
  return {
    ...
    promise: () => request.put('zashiki-transport/embark/store', route)
  }
}
...
export function store ({ statement }) {
  ...
  .then(() => (
    dispatch(embarkRoute({ response: { embark: Rails.part(statement) } })))
  )
}
